...
export function Viewer({ roomCode, onLeave }) {
  ...
  // Timer reconstruction — segment timer + show clock
  useEffect(() => {
    const tick = () => {
      const r = remoteRef.current;
      if (!r) return;
      // Segment timer
      if (r.running && r.mst) setLocalMs((r.saved || 0) + (Date.now() - r.mst));
      else if (r.running && !r.mst) setLocalMs(r.ms || 0);
      else setLocalMs(r.ms || 0);
      // Show clock
      const sc = r.showClock;
      if (sc?.running && sc?.mst) setShowClockMs((sc.saved || 0) + (Date.now() - sc.mst));
      else setShowClockMs(sc?.saved || 0);
    };
    tick();
  ...
  // Helper: get camera timer remaining
  const getCamTimer = (cam) => {
    const ct = remote?.camTimers?.[cam];
  ...
  const { mode, running, cdTotal, tallies, camNames: rNames, messages } = remote;
  ...
  const scf = fmt(showClockMs);
  const showClockStr = `${scf.h}:${scf.m}:${scf.s}`;
  const scColor = remote.showClock?.running ? "#8be9fd" : showClockMs > 0 ? "#ffb86c" : "#555";

  // ── Show clock bar (shared across all views) ──
  const showClockBar = showClockMs > 0 || remote.showClock?.running ? (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 12px", background: "#0c0c16", borderBottom: "1px solid #1a1a2e" }}>
      <span style={{ fontFamily: FF, fontSize: "0.45rem", color: scColor, letterSpacing: "0.2em" }}>
        {remote.showClock?.running ? "● SHOW" : "SHOW ⏸"}
      </span>
      <span style={{ fontFamily: FF, fontSize: "0.75rem", color: scColor, letterSpacing: "0.08em", fontWeight: 500 }}>{showClockStr}</span>
    </div>
  ) : null;
            ...
            {Object.entries(tallies).map(([cam, st]) => {
              ...
              return (
                <button key={cam} onClick={() => setViewMode(cam)} style={{
                  ...
                  <div style={{ fontFamily: FF, fontSize: "0.9rem", color: "#ddd", letterSpacing: "0.08em", fontWeight: 500 }}>{rNames?.[cam] || cam.toUpperCase().replace("CAM", "CAM ")}</div>
                  <div style={{ fontFamily: FF, fontSize: "0.5rem", marginTop: "4px", letterSpacing: "0.2em", color: isPgm ? "#ff5555" : isPvw ? "#50fa7b" : "#444" }}>{isPgm ? "● PGM" : isPvw ? "● PVW" : "OFF"}</div>
                </button>
              );
            })}
          </div>
  ...
  if (viewMode && viewMode !== "all" && tallies) {
    ...
    const fullLabel = isPgm ? "PROGRAM" : isPvw ? "PREVIEW" : t("viewer_off_air");
    const camName = rNames?.[cam] || cam.toUpperCase().replace("CAM", "CAM ");
    const ctd = getCamTimer(cam);
    ...
    return (
      <div style={{
        ...
        {/* Tally dot */}
        <div style={{ width: isPgm ? "20px" : "14px", height: isPgm ? "20px" : "14px", borderRadius: "50%", background: fullColor, boxShadow: st !== "off" ? `0 0 20px ${fullColor}, 0 0 60px ${fullColor}44` : "none", animation: isPgm ? "blink 1s ease-in-out infinite" : "none", marginBottom: "16px" }} />

        {/* Camera name */}
        <div style={{ fontFamily: FF, fontSize: "clamp(2.5rem, 14vw, 6rem)", color: "#fff", letterSpacing: "0.08em", fontWeight: 500, textAlign: "center", textShadow: st !== "off" ? `0 0 40px ${fullColor}33` : "none", padding: "0 20px" }}>{camName}</div>

        {/* Status */}
        <div style={{ fontFamily: FF, fontSize: "clamp(0.9rem, 4vw, 1.4rem)", color: fullColor, letterSpacing: "0.4em", marginTop: "8px", fontWeight: 500 }}>{fullLabel}</div>

        {/* Camera countdown timer */}
        {ctd && (
          <div style={{ marginTop: "16px", padding: "8px 20px", border: `2px solid ${ctd.expired ? "#ff5555" : "#f1fa8c"}44`, borderRadius: "8px", background: "#00000044" }}>
            <div style={{ fontFamily: FF, fontSize: "clamp(1.4rem, 6vw, 2.5rem)", color: ctd.expired ? "#ff5555" : ctd.remaining <= 30000 ? "#ffb86c" : "#f1fa8c", letterSpacing: "0.08em", textAlign: "center", animation: ctd.expired ? "blink 1s infinite" : "none" }}>
        ...
        {msg?.text && (
          <div style={{ marginTop: "16px", padding: "12px 24px", background: "#ffb86c22", border: "2px solid #ffb86c88", borderRadius: "8px", maxWidth: "90vw" }}>
            <div style={{ fontFamily: FF, fontSize: "clamp(1rem, 5vw, 1.8rem)", color: "#ffb86c", letterSpacing: "0.08em", textAlign: "center", animation: "blink 2s infinite", fontWeight: 500 }}>
              {msg.text}
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
            {(showClockMs > 0 || remote.showClock?.running) && (
              <div style={{ fontFamily: FF, fontSize: "0.5rem", color: scColor, letterSpacing: "0.08em" }}>
                SHOW {showClockStr}
              </div>
            )}
            <div style={{ fontFamily: FF, fontSize: "0.5rem", color: timerColor, letterSpacing: "0.15em", animation: running ? "blink 2s infinite" : "none" }}>{timerStatus}</div>
          </div>
          <div style={{ fontFamily: FF, fontSize: "clamp(1rem, 4vw, 1.5rem)", color: timerColor, letterSpacing: "0.08em", fontWeight: 500, textShadow: `0 0 15px ${timerColor}22` }}>{timerStr}</div>
        </div>

        {mode === "countdown" && rem <= 0 && !running && localMs > 0 && (
          <div style={{ position: "fixed", top: "4px", left: "4px", right: "4px", padding: "10px", background: "#ff555533", border: "2px solid #ff5555", borderRadius: "6px", textAlign: "center", fontFamily: FF, color: "#ff5555", fontSize: "0.85rem", letterSpacing: "0.35em", animation: "blink 1s infinite", zIndex: 100 }}>{t("master_time_up")}</div>
        )}
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div className="safe-top" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", borderBottom: "1px solid #12121e" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          <button onClick={() => setViewMode(null)} style={{ padding: "5px 10px", border: "1px solid #333", borderRadius: "3px", background: "transparent", color: "#888", fontFamily: FF, fontSize: "0.5rem", cursor: "pointer", letterSpacing: "0.1em" }}>{t("viewer_change")}</button>
          <Btn small color="#ff555555" onClick={onLeave}>{t("master_exit")}</Btn>
        </div>
      </div>
      {showClockBar}
            ...
            {Object.entries(tallies).map(([cam, st]) => {
              ...
              return (
                <div key={cam} onClick={() => setViewMode(cam)} style={{
                  ...
                  borderRadius: "8px", padding: "14px 8px", textAlign: "center", cursor: "pointer",
                  ...
                  <div style={{ fontFamily: FF, fontSize: "0.5rem", color: isPgm ? "#ff5555" : isPvw ? "#50fa7b" : "#666", letterSpacing: "0.25em", marginTop: "4px", animation: isPgm ? "blink 1.2s infinite" : "none" }}>
                    {isPgm ? t("tally_program") : isPvw ? t("tally_preview") : t("tally_off")}
                  </div>
                  {/* Camera timer */}
                  {ctd && (
                    <div style={{ fontFamily: FF, fontSize: "0.7rem", color: ctd.expired ? "#ff5555" : "#f1fa8c", marginTop: "4px", animation: ctd.expired ? "blink 1s infinite" : "none" }}>
                      ⏱ {(() => { const f = fmt(ctd.remaining); return `${f.m}:${f.s}`; })()}
                    </div>
                  )}
                  {/* Message */}
                  {msg?.text && (
                    <div style={{ marginTop: "4px", padding: "3px 6px", background: "#ffb86c18", borderRadius: "3px" }}>
                      <div style={{ fontFamily: FF, fontSize: "0.5rem", color: "#ffb86c", animation: "blink 2s infinite" }}>{msg.text}</div>
                    </div>
                  )}
                </div>
              );
      ...
      {mode === "countdown" && rem <= 0 && !running && localMs > 0 && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, padding: "8px", background: "#ff555528", borderBottom: "2px solid #ff5555", textAlign: "center", fontFamily: FF, color: "#ff5555", fontSize: "0.8rem", letterSpacing: "0.3em", animation: "blink 1s infinite", zIndex: 100 }}>{t("master_time_up")}</div>
      )}
    </div>
  );
}
